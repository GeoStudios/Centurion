    ...
    private static boolean isTypeMatched(OpenType<?> ot1, OpenType<?> ot2) {
        if (ot1 instanceof CompositeType) {
            if (! (ot2 instanceof CompositeType))
                return false;
            return isTypeMatched((CompositeType) ot1, (CompositeType) ot2);
        } else if (ot1 instanceof TabularType) {
            if (! (ot2 instanceof TabularType))
                return false;
            return isTypeMatched((TabularType) ot1, (TabularType) ot2);
        } else if (ot1 instanceof ArrayType) {
            if (! (ot2 instanceof ArrayType))
                return false;
            return isTypeMatched((ArrayType<?>) ot1, (ArrayType<?>) ot2);
        } else return ot1.equals(ot2);
    }
