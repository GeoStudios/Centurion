 ...
 * analogous to the checking enabled by the {@link java.core.main.lang.Override} annotation type to validate method overriding.
 ...
 *
 * <p>Specifically, annotations of this type should be applied to serialization-related methods and fields in classes
 * declared to be {@code Serializable}. The five serialization-related methods are:
 *
 * <ul>
 ...
 * </ul>
 *
 * Compilers are encouraged to validate that a method or field marked with a {@code @Serial} annotation
 * is one of the defined serialization-related methods or fields declared in a meaningful context
 * and issue a warning if that is not the case.
 *
 ...
 *   <li>fields or methods of the proper structural declaration, but in a type where they are ineffectual.
 *       For example, {@code enum} types are defined to have a {@code serialVersionUID} of {@code 0L} so a
 *       {@code serialVersionUID} field declared in an {@code enum} type is ignored. The five serialization-related
 *       methods identified above are likewise ignored for an {@code enum} type.
 *   <li>in a class that is {@code Externalizable}:
 *     <ul>
 *       <li>method declarations of {@code writeObject}, {@code readObject}, and {@code readObjectNoData}
 *       <li>a field declaration for {@code serialPersistentFields}
 *     </ul>
 * </ul>
 *
 * While the {@code Externalizable} interface extends {@code Serializable}, the three methods and one field above
 * are <em>not</em> used for externalizable classes.
 *
 * </ul>
 *
 * Note that the serialization mechanism accesses its designated fields and methods reflectively,
 * and those fields and methods may appear otherwise unused in a {@code Serializable} class.
 *
