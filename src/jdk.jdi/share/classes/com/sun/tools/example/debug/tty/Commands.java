...
import jdk.jdi.share.classes.com.sun.tools.example.debug.expr.ExpressionParser;
import jdk.jdi.share.classes.com.sun.tools.example.debug.expr.ParseException;
import java.text.*;
import java.util.*;
import java.io.*;
...
class Commands {
    ...
    String typedName(Method method) {
        ...
        if (lastParam >= 0) {
            ...
            if (method.isVarArgs()) {
                ...
                sb.append(lastStr, 0, lastStr.length() - 2);
                sb.append("...");
            } else {
                sb.append(lastStr);
            }
    ...
    void commandClass(StringTokenizer t) {
        ...
        if (t.hasMoreTokens()) {
            if (t.nextToken().equalsIgnoreCase("all")) {
                showAll = true;
            } else {
    ...
    void commandStep(StringTokenizer t) {
        ...
        int depth;
        if (t.hasMoreTokens() &&
                  t.nextToken().equalsIgnoreCase("up")) {
    ...
    void commandClasspath(StringTokenizer t) {
        if (Env.vm() instanceof PathSearchingVirtualMachine vm) {
    ...
    void doPrint(StringTokenizer t, boolean dumpObject) {
        ...
        while (t.hasMoreTokens()) {
            ...
            if (val == null) {
            ...
            } else if (dumpObject && (val instanceof ObjectReference obj) &&
                ...
                MessageOutput.println("grouping end character");
                  ...
                  if (strVal != null) {
                     MessageOutput.println("expr is value", new Object [] {expr,
                                                                      strVal});
                   }
            }
        }
    ...
    void doLock(StringTokenizer t) {
        ...
        try {
            if ((val != null) && (val instanceof ObjectReference object)) {
                String strVal = getStringValue();
                if (strVal != null) {
