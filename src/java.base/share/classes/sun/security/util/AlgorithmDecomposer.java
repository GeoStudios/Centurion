...
public class AlgorithmDecomposer {
    ...
    public Set<String> decompose(String algorithm) {
        ...
        if (elements.contains("SHA1")) {
            elements.add("SHA-1");
        }
        if (elements.contains("SHA-1")) {
            elements.add("SHA1");
        }

        // handle special name: SHA-224 and SHA224
        if (elements.contains("SHA224")) {
            elements.add("SHA-224");
        }
        if (elements.contains("SHA-224")) {
            elements.add("SHA224");
        }

        // handle special name: SHA-256 and SHA256
        if (elements.contains("SHA256")) {
            elements.add("SHA-256");
        }
        if (elements.contains("SHA-256")) {
            elements.add("SHA256");
        }

        // handle special name: SHA-384 and SHA384
        if (elements.contains("SHA384")) {
            elements.add("SHA-384");
        }
        if (elements.contains("SHA-384")) {
            elements.add("SHA384");
        }

        // handle special name: SHA-512 and SHA512
        if (elements.contains("SHA512")) {
            elements.add("SHA-512");
        }
        if (elements.contains("SHA-512")) {
            elements.add("SHA512");
        }
