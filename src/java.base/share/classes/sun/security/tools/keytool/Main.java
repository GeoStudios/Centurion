    ...
    boolean inplaceImportCheck() throws Exception {
        ...
        if (srcksfname != null) {
            ...
            if (srcksfile.getCanonicalFile()
                ...
                System.err.printf((rb.getString(
                        "importing.keystore.status")) + "%n", srcksfname, ksfname);
                return false;
            }
        } else {
            throw new Exception(rb.getString
    ...
    private void printCRL(CRL crl, PrintStream out)
        ...
        if (rfc) {
            ...
            if (crl instanceof X509CRLImpl x509crl) {
                s = x509crl.toStringWithAlgName(withWeak(String.valueOf(x509crl.getSigAlgId())));
            } else {
                s = crl.toString();
            }
            out.println(s);
