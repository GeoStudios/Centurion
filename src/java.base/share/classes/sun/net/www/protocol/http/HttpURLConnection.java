...
public class HttpURLConnection extends java.net.HttpURLConnection {
    ...
    private boolean isRestrictedHeader(String key, String value) {
        ...
        } else return key.startsWith("sec-");
    }
                    ...
                    while (it.hasNext()) {
                        ...
                        try {
                            ...
                            if (p != Proxy.NO_PROXY) {
                                ...
                                if (!it.hasNext()) {
                                    if (logger.isLoggable(PlatformLogger.Level.FINEST)) {
                                        logger.finest("Retrying with proxy: " + p);
                                    }
                                    http = getNewHttpClient(url, p, connectTimeout, false);
                                    http.setReadTimeout(readTimeout);
                                    break;
    ...
    private void setCookieHeader() throws IOException {
        if (cookieHandler != null) {
            ...
            if (uri != null) {
                ...
                if (!cookies.isEmpty()) {
                    if (logger.isLoggable(PlatformLogger.Level.FINEST)) {
                        logger.finest("Cookies retrieved: " + cookies);
                    }
        ...
        try {
            do {
                ...
                if (serverAuthentication != null) {
                    ...
                    if (!(serverAuthentication instanceof DigestAuthentication srv) ||
                        (domain == null)) {
                ...
                java.security.AccessController.doPrivileged(
                    new java.security.PrivilegedExceptionAction<>() {
                        public IOException run() throws Exception {
                            return rememberedException.getClass()
    ...
    private AuthenticationInfo
        ...
        if ((proxyAuthentication != null )&&
            ...
            if (proxyAuthentication.isAuthorizationStale (raw)) {
                /* we can retry with the current credentials */
                String value;
                if (proxyAuthentication instanceof DigestAuthentication digestProxy) {
    ...
    private void setPreemptiveProxyAuthentication(MessageHeader requests) throws IOException {
        ...
        if (pauth != null && pauth.supportsPreemptiveAuthorization()) {
            String value;
            if (pauth instanceof DigestAuthentication digestProxy) {
                if (tunnelState() == TunnelState.SETUP) {
                    value = digestProxy
    ...
    private AuthenticationInfo getHttpProxyAuthentication(AuthenticationHeader authhdr) {
        ...
        if (logger.isLoggable(PlatformLogger.Level.FINER)) {
            logger.finer("Proxy Authentication for " + authhdr +" returned " + (ret != null ? ret.toString() : "null"));
        }
        return ret;
    }
    ...
    private AuthenticationInfo getServerAuthentication(AuthenticationHeader authhdr) {
        ...
        if (logger.isLoggable(PlatformLogger.Level.FINER)) {
            logger.finer("Server Authentication for " + authhdr +" returned " + (ret != null ? ret.toString() : "null"));
        }
        return ret;
    }
    ...
    public Map<String, List<String>> getRequestProperties() {
        ...
        try {
            ...
            if (userCookies != null || userCookies2 != null) {
                ...
                if (userCookies != null) {
                    userCookiesMap.put("Cookie", List.of(userCookies));
                }
                if (userCookies2 != null) {
                    userCookiesMap.put("Cookie2", List.of(userCookies2));
                }
            }
