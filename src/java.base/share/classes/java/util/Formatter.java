...
public final class Formatter implements Closeable, Flushable {
    ...
    private class FormatSpecifier implements FormatString {
        ...
        private void flags(String s, int start, int end) {
            ...
            if (Flags.contains(Flags.PREVIOUS))
                index = -1;
        }
        ...
        private void printString(Object arg, Locale l) throws IOException {
            if (arg instanceof Formattable) {
                ...
                if (Flags.contains(Flags.ALTERNATE))
        ...
        private void appendJustified(Appendable a, CharSequence cs) throws IOException {
             ...
             boolean padRight = Flags.contains(Flags.LEFT_JUSTIFY);
        ...
        public String toString() {
            ...
            if (dt)
                sb.append(Flags.contains(Flags.UPPERCASE) ? 'T' : 't');
            sb.append(Flags.contains(Flags.UPPERCASE)
        ...
        private void checkGeneral() {
            if ((c == Conversion.BOOLEAN || c == Conversion.HASHCODE)
                && Flags.contains(Flags.ALTERNATE))
                failMismatch(Flags.ALTERNATE, c);
            // '-' requires a width
            if (width == -1 && Flags.contains(Flags.LEFT_JUSTIFY))
        ...
        private void checkNumeric() {
            ...
            if (width == -1
                && (Flags.contains(Flags.LEFT_JUSTIFY) || Flags.contains(Flags.ZERO_PAD)))
                throw new MissingFormatWidthException(toString());

            // bad combination
            if ((Flags.contains(Flags.PLUS) && Flags.contains(Flags.LEADING_SPACE))
                || (Flags.contains(Flags.LEFT_JUSTIFY) && Flags.contains(Flags.ZERO_PAD)))
        ...
        private void print(long value, Locale l) throws IOException {
            ...
            if (c == Conversion.DECIMAL_INTEGER) {
                ...
                int len = (Flags.contains(Flags.ALTERNATE)
                           ? s.length() + 1
                           : s.length());
                ...
                if (Flags.contains(Flags.ALTERNATE))
                    sb.append(Flags.contains(Flags.UPPERCASE) ? "0X" : "0x");
                if (Flags.contains(Flags.ZERO_PAD)) {
                    trailingZeros(sb, width - len);
                }
                if (Flags.contains(Flags.UPPERCASE))
        ...
        private StringBuilder leadingSign(StringBuilder sb, boolean neg) {
            if (!neg) {
                if (Flags.contains(Flags.PLUS)) {
                    sb.append('+');
                } else if (Flags.contains(Flags.LEADING_SPACE)) {
                    sb.append(' ');
                }
            } else {
        ...
        private StringBuilder trailingSign(StringBuilder sb, boolean neg) {
            if (neg && Flags.contains(Flags.PARENTHESES))
                sb.append(')');
            return sb;
        }
        ...
        private void print(double value, Locale l) throws IOException {
            ...
            if (!Double.isNaN(value)) {
                ...
                sb.append(Flags.contains(Flags.UPPERCASE) ? "NAN" : "NaN");
            }
            ...
            throws IOException
        {
            if (c == Conversion.SCIENTIFIC) {
                ...
                if (Flags.contains(Flags.ZERO_PAD)) {
                    ...
                    if(Flags.contains(Flags.LEADING_SPACE) ||
                            Flags.contains(Flags.PLUS) || neg) {
                        leadingCharacters = 3;
                    }
            ...
            throws IOException
        {
            if (c == Conversion.SCIENTIFIC) {
                ...
                if ((origPrec == 1 || !bdl.hasDot())
                        && (nzeros > 0 || (Flags.contains(Flags.ALTERNATE)))) {
                    mant.append('.');
                }
                ...
                // requested precision.
                if (bdl.scale() == 0 && (Flags.contains(Flags.ALTERNATE)
                        || nzeros > 0)) {
                    mant.append('.');
                }
        ...
        private int adjustWidth(int width, Flags f, boolean neg) {
            int newW = width;
            if (newW != -1 && neg && Flags.contains(Flags.PARENTHESES))
                newW--;
            return newW;
        }
