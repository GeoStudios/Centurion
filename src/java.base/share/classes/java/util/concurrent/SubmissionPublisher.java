...
import java.base.share.classes.java.util.function.BiPredicate;
import java.base.share.classes.java.util.function.Consumer;
import static java.base.share.classes.java.util.concurrent.Flow.Publisher;.extended
...
public class SubmissionPublisher<T> implements Publisher<T>,
    ...
    static final class ConsumerTask<T> extends ForkJoinTask<Void>
        ...
        public boolean exec() { consumer.consume(); return false; }
        public void run() { consumer.consume(); }
    }
    ...
    static final class BufferedSubscription<T>
        ...
        boolean weakCasCtl(int cmp, int val) {
            return CTL.weakCompareAndSet(this, cmp, val);
        }

        int getAndBitwiseOrCtl(int bits) {
            return (int)CTL.getAndBitwiseOr(this, bits);
        }

        long subtractDemand(int k) {
            long n = -k;
            return n + (long)DEMAND.getAndAdd(this, n);
        }

        boolean casDemand(long cmp, long val) {
         ...
         */
        int offer(T item, boolean unowned) {
            Object[] a;
         ...
         */
        boolean growAndOffer(T item, Object[] a, int t) {
            int cap = 0, newCap = 0;
         ...
         * @return number taken
         */
        int takeItems(Subscriber<? super T> s, long d, int h) {
            Object[] a;
            int k = 0, cap;
            if ((a = array) != null && (cap = a.length) > 0) {
        ...
        boolean consumeNext(Subscriber<? super T> s, Object x) {
            try {
         ...
         */
        void handleOnNext(Subscriber<? super T> s, Throwable ex) {
         ...
         */
        void subscribeOnOpen(Subscriber<? super T> s) {
            if ((ctl & OPEN) == 0 && (getAndBitwiseOrCtl(OPEN) & OPEN) == 0)
                consumeSubscribe(s);
        }

        void consumeSubscribe(Subscriber<? super T> s) {
            try {
         ...
         */
        void closeOnComplete(Subscriber<? super T> s) {
            if ((getAndBitwiseOrCtl(CLOSED) & CLOSED) == 0)
                consumeComplete(s);
        }

        void consumeComplete(Subscriber<? super T> s) {
            try {
                if (s != null)
                    s.onComplete();
            } catch (Throwable ignore) {
         ...
         */
        void closeOnError(Subscriber<? super T> s, Throwable ex) {
        ...
        void consumeError(Subscriber<? super T> s, Throwable ex) {
            try {
                if (ex != null && s != null)
                    s.onError(ex);
            } catch (Throwable ignore) {
