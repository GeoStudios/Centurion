...
public class Checker extends DocTreePathScanner<Void, Void> {
    ...
    private void checkStructure(StartElementTree tree, HtmlTag t) {
        ...
        switch (t.blockType) {
            ...
            case OTHER:
                if (t == HtmlTag.SCRIPT) {// <script> may or may not be allowed, depending on --allow-script-in-comments
    ...
    private boolean isConstant(Element e) {
        ...
        if (Objects.requireNonNull(e.getKind()) == ElementKind.FIELD) {
    ...
    private boolean isSynthetic() {
        if (Objects.requireNonNull(env.currElement.getKind()) == ElementKind.CONSTRUCTOR) {// A synthetic default constructor has the same pos as the
            // enclosing class
    ...
    void warnIfEmpty(DocTree tree, List<? extends DocTree> list) {
        for (DocTree d: list) {
            if (Objects.requireNonNull(d.getKind()) == DocTree.Kind.TEXT) {
