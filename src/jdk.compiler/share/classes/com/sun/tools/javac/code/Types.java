...
public class Types {
    ...
    class DescriptorCache {
        ...
        private final WeakHashMap<TypeSymbol, Entry> _map = new WeakHashMap<>();
    ...
    private final DescriptorCache descCache = new DescriptorCache();
    ...
    //where
        ...
        private final Predicate<Symbol> bridgeFilter = new Predicate<Symbol>() {
    ...
    */
    ...
    // where
        private final TypeRelation isSubtype = new TypeRelation()
        {
            @Override
   ...
   public boolean isSignaturePolymorphic(MethodSymbol msym) {
    ...
    // where
        private final TypeRelation containsType = new TypeRelation() {
...
//                    debugContainsType(t, s);
    ...
    public boolean isCastable(Type t, Type s, Warner warn) {
        ...
        if (result && t.hasTag(CLASS) && t.tsym.kind.matches(Kinds.KindSelector.TYP)
            ...
            return !t.isCompound() && !s.isCompound() && !areDisjoint((ClassSymbol) t.tsym, (ClassSymbol) s.tsym);
        }
        return result;
    }
    // where
        ...
        private final TypeRelation isCastable = new TypeRelation() {
    ...
    // where
        private final TypeRelation disjointType = new TypeRelation() {
    ...
    // where
        private final UnaryVisitor<Boolean> isReifiable = new UnaryVisitor<Boolean>() {
     ...
     */
    private final TypeMapping<Void> elemTypeFun = new TypeMapping<Void>() {
        @Override
    ...
    public ArrayType makeArrayType(Type t) {
        if (t.hasTag(VOID) || t.hasTag(PACKAGE)) {
            Assert.error("Type t must not be a VOID or PACKAGE type, " + t);
        }
    ...
    // where
        private final SimpleVisitor<Type,Symbol> asSuper = new SimpleVisitor<Type,Symbol>() {

            private final Set<Symbol> seenTypes = new HashSet<>();
    ...
    // where
        private final SimpleVisitor<Type,Symbol> memberType = new SimpleVisitor<Type,Symbol>() {
    ...
    // where
        private final TypeMapping<Boolean> erasure = new StructuralTypeMapping<Boolean>() {
    ...
    // where
        private final UnaryVisitor<Type> supertype = new UnaryVisitor<Type>() {
    ...
    // where
        private final UnaryVisitor<List<Type>> interfaces = new UnaryVisitor<List<Type>>() {
    ...
    // where
        private final UnaryVisitor<Type> classBound = new UnaryVisitor<Type>() {
    ...
    class ImplementationCache {
        ...
        private final WeakHashMap<MethodSymbol, SoftReference<Map<TypeSymbol, Entry>>> _map = new WeakHashMap<>();
    ...
    private final ImplementationCache implCache = new ImplementationCache();
    ...
    class MembersClosureCache extends SimpleVisitor<Scope.CompoundScope, Void> {
        ...
        private final Map<TypeSymbol, CompoundScope> _map = new HashMap<>();
    ...
    private final MembersClosureCache membersCache = new MembersClosureCache();
    ...
    // where
        private class HasSameArgs extends TypeRelation {
            ...
            public Boolean visitForAll(ForAll t, Type s) {
                if (!s.hasTag(FORALL))
                    return !strict && visitMethodType(t.asMethodType(), s);
     ...
     * otherwise).
     */
    private final Map<Type,List<Type>> closureCache = new HashMap<>();
