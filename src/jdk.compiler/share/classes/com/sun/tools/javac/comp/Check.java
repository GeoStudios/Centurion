...
public class Check {
     ...
     */
    private final Map<Pair<ModuleSymbol, Name>,ClassSymbol> compiled = new HashMap<>();

    /** A handler for messages about deprecated usage.
     */
    private final MandatoryWarningHandler deprecationHandler;

    /** A handler for messages about deprecated-for-removal usage.
     */
    private final MandatoryWarningHandler removalHandler;

    /** A handler for messages about unchecked or unsafe usage.
     */
    private final MandatoryWarningHandler uncheckedHandler;

    /** A handler for messages about using proprietary API.
     */
    private final MandatoryWarningHandler sunApiHandler;
 ...
 **************************************************************************/
    ...
    private final Predicate<Symbol> equalsHasCodeFilter = s -> MethodSymbol.implementation_filter.test(s) &&
            ...
            while (qualId != null) {
                ...
                switch (qualId.getTag()) {
                    ...
                    case IDENT:
                        ...
                        break;
                    default:
                        throw new AssertionError("Unexpected qualified identifier: " + qualId);
                }
    ...
    void checkNonCyclicElementsInternal(DiagnosticPosition pos, TypeSymbol tsym) {
        ...
        try {
            ...
            for (Symbol s : tsym.members().getSymbols(NON_RECURSIVE)) {
                if (s.kind != MTH)
                    continue;
                checkAnnotationResType(pos, s.type.getReturnType());
            }
        } finally {
            tsym.flags_field &= ~LOCKED;
