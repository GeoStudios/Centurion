...
import jdk.compiler.share.classes.com.sun.tools.javac.code.Symbol.OperatorSymbol.AccessCode;
 ...
 *************************************************************************/
    ...
    ClassSymbol accessConstructorTag() {
        ...
        for (int i = 1; ; i++) {
            Name flatname = names.fromString(String.valueOf(topClass.getQualifiedName()) +
                                            target.syntheticNameChar() +
                                            i);
 ...
 *************************************************************************/
    ...
    Name proxyName(Name name, int index) {
        ...
        if (index > 0) {
            proxyName = proxyName.append(names.fromString(String.valueOf(target.syntheticNameChar()) + index));
        }
        return proxyName;
    }
    ...
    Name outerThisName(Type type, Symbol owner) {
        ...
        while (owner.kind == TYP && owner.members().findFirst(result) != null)
    ...
    private void visitEnumDef(JCClassDecl tree) {
        ...
        private Name syntheticName(JCClassDecl tree, String baseName) {
            ...
            while (tree.sym.members().findFirst(valuesName) != null) // avoid name clash
                valuesName = names.fromString(valuesName + String.valueOf(target.syntheticNameChar()));
            return valuesName;
        }
        ...
        for (JCCase c : cases) {
            if (c.labels.head.isExpression()) {
                ...
                if (TreeInfo.isNull(c.labels.head)) {
                    ...
                    VarSymbol label = (VarSymbol)TreeInfo.symbol(c.labels.head);
