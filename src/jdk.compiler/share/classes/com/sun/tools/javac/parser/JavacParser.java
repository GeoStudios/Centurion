...
public class JavacParser implements Parser {
    ...
    private final JCErroneous errorTree;
...
/* ---------- parsing -------------- */
    ...
    private List<JCCase> switchExpressionStatementGroup() {
        ...
        if (Objects.requireNonNull(token.kind) == ARROW) {
            checkSourceLevel(Feature.SWITCH_RULE);
            nextToken();
            if (token.kind == TokenKind.THROW || token.kind == LBRACE) {
    ...
    JCExpression arguments(List<JCExpression> typeArgs, JCExpression t) {
        ...
        if (t.hasTag(IDENT) && isInvalidUnqualifiedMethodIdentifier(t.pos,
                                                                    ((JCIdent) t).name)) {
