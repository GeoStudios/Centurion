...
public class Gen extends JCTree.Visitor {
    ...
    private final boolean disableVirtualizedPrivateInvoke;
 ...
 *************************************************************************/
    ...
    private final ClassReferenceVisitor classReferenceVisitor = new ClassReferenceVisitor();
        ...
        if (cases.isEmpty()) {
            ...
            for (int i = 0; i < labels.length; i++) {
                if (l.head.labels.head.isExpression()) {
                    ...
                    int val = ((Number) l.head.labels.head.type.constValue()).intValue();
                    labels[i] = val;
                    if (val < lo) lo = val;
                    if (hi < val) hi = val;
                    nlabels++;
...
//where
    ...
    public boolean isInvokeDynamic(Symbol sym) {
        return sym.kind == MTH && sym.isDynamic();
    }
