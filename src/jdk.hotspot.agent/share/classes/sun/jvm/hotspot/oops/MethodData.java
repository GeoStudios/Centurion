...
public class MethodData extends Metadata implements MethodDataInterface<Klass,Method> {
  ...
  public long[] data() {
    ...
    for (int i = 0; i < elements; i++) {
      Address value = base.getAddressAt(offset + (long) i * MethodData.cellSize);
      if (value != null) {
  ...
  int dumpReplayDataTypeHelper(PrintStream out, int round, int count, int index, ProfileData pdata, Klass k) {
    if (k != null) {
      if (round == 0) count++;
                     ...
                     (dpToDi(pdata.dp() +
                             ProfileData.cellOffset(index)) / cellSize) + " " +
                     k.getName().asString());
    }
    return count;
  }
      ...
      case DataLayout.speculativeTrapDataTag: {
        ...
        if (m != null) {
          if (round == 0) {
            ...
            out.print(" " +  (dpToDi(data.dp() + ProfileData.cellOffset(SpeculativeTrapData.methodIndex())) / cellSize) + " " +  m.nameAsAscii());
          }
        }
        break;
      }
