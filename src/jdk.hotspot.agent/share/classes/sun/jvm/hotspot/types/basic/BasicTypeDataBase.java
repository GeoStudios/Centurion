...
public class BasicTypeDataBase implements TypeDataBase {
  private final MachineDescription machDesc;
  private final VtblAccess vtblAccess;
  /** Maps strings to Type objects. This does not contain the primitive types. */
  private final Map<String, Type> nameToTypeMap = new HashMap<>();
  /** Maps strings to Integers, used for enums, etc. */
  private final Map<String, Integer> nameToIntConstantMap = new HashMap<>();
  /** Maps strings to Longs, used for 32/64-bit constants, etc. */
  private final Map<String, Long> nameToLongConstantMap = new HashMap<>();
  /** Primitive types. */
  ...
  public Type findDynamicTypeForAddress(Address addr, Type baseType) {
    ...
    for (Iterator iter = getTypes(); iter.hasNext(); ) {
      ...
      if (loc2Match == null && vtblAddr.equals(loc2)) {
          loc2Match = type;
      }
      if (loc3Match == null && vtblAddr.equals(loc3)) {
          loc3Match = type;
      }
    }
