...
public class CommandProcessor {
    ...
    public static class NonBootFilter implements ClassFilter {
        private final HashMap<Symbol, InstanceKlass> emitted = new HashMap<>();
        public boolean canInclude(InstanceKlass kls) {
    ...
    private final Command[] commandList = {
        ...
        new Command("examine", "examine [ address/count ] | [ address,address]", false) {
            final Pattern args1 = Pattern.compile("^(0x[0-9a-f]+)(/([0-9]*)([a-z]*))?$");
            final Pattern args2 = Pattern.compile("^(0x[0-9a-f]+),(0x[0-9a-f]+)(/[a-z]*)?$");
            ...
            public void doit(Tokens t) {
                if (t.countTokens() != 1) {
                    ...
                    if (m1.matches()) {
                        ...
                        end = start.addOffsetTo((long) count * formatSize);
                    } else if (m2.matches()) {
            ...
            public void doit(Tokens t) {
                ...
                for (int i = 0; i < threads.getNumberOfThreads(); i++) {
                    ...
                    if (all || bos.toString().equals(name)) {
                        if (thread instanceof CompilerThread ct) {
                            ciEnv env = ct.env();
                            if (env != null) {
        ...
        new Command("jstack", "jstack [-v]", false) {
            public void doit(Tokens t) {
                boolean verbose = t.countTokens() > 0 && t.nextToken().equals("-v");
                        ...
                        if (RobustOopDeterminator.oopLooksValid(handle)) {
                            try {
                                ...
                                if (klass == null || oop.getKlass().isSubtypeOf(klass))
                                    out.println(handle + " " + oop.getKlass().getName().asString());
                    ...
                    for (int i = 0; i < threads.getNumberOfThreads(); i++) {
                        ...
                        while (base != null && base.lessThan(end)) {
                            ...
                            if (AddressOps.equal(val, value)) {
                                ByteArrayOutputStream bos = new ByteArrayOutputStream();
                                thread.printThreadIDOn(new PrintStream(bos));
                                out.println("found on the stack of thread " + bos + " at " + base);
                            }
            ...
            public void doit(Tokens t) {
                if (t.countTokens() != 1) {
                    ...
                    for (int i = 0; i < threads.getNumberOfThreads(); i++) {
                        ...
                        thread.printThreadIDOn(new PrintStream(bos));
                        if (all || bos.toString().equals(name)) {
                            out.println("Thread " + bos + " Address: " + thread.getAddress());
                            HTMLGenerator gen = new HTMLGenerator(false);
                            try {
    ...
    private final DebuggerInterface debugger;
    private final HotSpotAgent agent;
        ...
        if (ln.indexOf('!') != -1) {
            ...
            if (size == 0) {
                ...
                while (m.find()) {
                    ...
                    if (m.start() > start) {
                        result.append(ln, start, m.start());
                    }
                    start = m.end();
