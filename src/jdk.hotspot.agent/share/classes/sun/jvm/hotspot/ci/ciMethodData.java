...
public class ciMethodData extends ciMetadata implements MethodDataInterface<ciKlass,ciMethod> {
  ...
  public  long[] data() {
    ...
    for (int i = 0; i < elements; i++) {
      Address value = base.getAddressAt((long) i * MethodData.cellSize);
      if (value != null) {
  ...
  int dumpReplayDataTypeHelper(PrintStream out, int round, int count, int index, ProfileData pdata, ciKlass k) {
    if (k != null) {
      if (round == 0) count++;
      else out.print(" " + ((pdata.dp() + ProfileData.cellOffset(index)) / MethodData.cellSize) + " " + k.name());
    }
    return count;
  }

  int dumpReplayDataReceiverTypeHelper(PrintStream out, int round, int count, ReceiverTypeData<ciKlass,ciMethod> vdata) {
    for (int i = 0; i < ReceiverTypeData.rowLimit(); i++) {
      ciKlass k = vdata.receiver(i);
      count = dumpReplayDataTypeHelper(out, round, count, ReceiverTypeData.receiverCellIndex(i), vdata, k);
    }
    return count;
  }
      ...
      case DataLayout.speculativeTrapDataTag: {
        ...
        if (m != null) {
          if (round == 0) {
            ...
            out.print(" " +  (dpToDi(data.dp() + ProfileData.cellOffset(SpeculativeTrapData.methodIndex())) / MethodData.cellSize) + " " +  m.nameAsAscii());
          }
        }
        break;
      }
