...
public class HeapHprofBinWriter extends AbstractHeapGraphWriter {
    ...
    private final HashSet<Symbol> names;
    ...
    private int getArrayHeaderSize(boolean isObjectAarray) {
        return isObjectAarray?
            ((int) BYTE_SIZE + 2 * (int) INT_SIZE + 2 * OBJ_ID_SIZE):
            (2 * (int) BYTE_SIZE + 2 * (int) INT_SIZE + OBJ_ID_SIZE);
    }
    ...
    private void writeClassDumpRecord(Klass k) throws IOException {
        ...
        if (k instanceof InstanceKlass) {
            ...
            if (k instanceof ObjArrayKlass oak) {
                Klass bottomKlass = oak.getBottomKlass();
                if (bottomKlass instanceof InstanceKlass ik) {
                    writeObjectID(ik.getClassLoader());
                    writeObjectID(null); // ik.getJavaMirror().getSigners());
                    writeObjectID(null); // ik.getJavaMirror().getProtectionDomain());
                } else {
    ...
    private void writeSymbol(Symbol sym) throws IOException {
        ...
        if (names.add(sym)) {
            if(sym != null) {
              byte[] buf = sym.asString().getBytes(StandardCharsets.UTF_8);
              writeHeader(HPROF_UTF8, buf.length + OBJ_ID_SIZE);
              writeSymbolID(sym);
              out.write(buf);
           } else {
              writeHeader(HPROF_UTF8, OBJ_ID_SIZE);
              writeSymbolID(null);
           }
        }
    }
    ...
    private final ArrayList<Klass> KlassMap;
    private final int gzLevel;
    ...
    private final Map<InstanceKlass, ClassData> classDataCache = new HashMap<>();
    ...
    private class SegmentedOutputStream extends BufferedOutputStream {
        ...
        private final boolean allowSegmented;
