...
abstract class DigestMD5Base extends AbstractSaslImpl {
    ...
    protected static byte[][] parseDirectives(byte[] buf,
        ...
        if (gettingQuotedValue) {
            throw new SaslException(
                "Unmatched quote found for directive: " + key +
                " with value: " + value);
        }
        ...
        public byte[] wrap(byte[] outgoing, int start, int len)
            ...
            try {
                ...
                if (cipherBlock == null) {
                    ...
                    throw new IllegalBlockSizeException(String.valueOf(toBeEncrypted.length));
                }
        ...
        public byte[] unwrap(byte[] incoming, int start, int len)
            ...
            try {
                ...
                if (decryptedMsg == null) {
                    // update() can return null
                    throw new IllegalBlockSizeException(String.valueOf(encryptedMsg.length));
                }
            } catch (IllegalBlockSizeException e) {
            ...
            if (blockSize > 1) {
                ...
                msgLength -= msgWithPadding[msgWithPadding.length - 1];
                if (msgLength < 0) {
                    //  Discard message and do not increment sequence number
