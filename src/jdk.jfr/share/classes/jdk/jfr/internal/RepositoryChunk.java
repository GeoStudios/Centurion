...
final class RepositoryChunk {
    ...
    public synchronized void use() {
        ++refCount;
        if (Logger.shouldLog(LogTag.JFR_SYSTEM, LogLevel.DEBUG)) {
            Logger.log(LogTag.JFR_SYSTEM, LogLevel.DEBUG, "Use chunk " + this + " ref count now " + refCount);
        }
    }
    ...
    public synchronized void release() {
        --refCount;
        if (Logger.shouldLog(LogTag.JFR_SYSTEM, LogLevel.DEBUG)) {
            Logger.log(LogTag.JFR_SYSTEM, LogLevel.DEBUG, "Release chunk " + this + " ref count now " + refCount);
        }
        if (refCount == 0) {
            destroy();
        }
    ...
    public boolean inInterval(Instant startTime, Instant endTime) {
        ...
        return endTime == null || !getStartTime().isAfter(endTime);
    }
