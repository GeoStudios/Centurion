...
public abstract class IntVector extends AbstractVector<Integer> {
    ...
    IntVector lanewiseTemplate(VectorOperators.Binary op,
        ...
        if (opKind(op, VO_SPECIAL  | VO_SHIFT)) {
            if (op == FIRST_NONZERO) {
                ...
                that = that.blend(0, thisNZ.cast(vspecies()));
                op = OR_UNCHECKED;
            }
            ...
            if (op == ROR || op == ROL) {  // FIXME: JIT should do this
                ...
                VectorMask<Integer> eqz = that.eq(0);
                if (eqz.anyTrue()) {
            ...
            BIN_IMPL.find(op, opc, (opc_) -> {
              switch (opc_) {
                ...
                case VECTOR_OP_URSHIFT: return (v0, v1) ->
                        v0.bOp(v1, (i, a, n) -> (a & LSHR_SETUP_MASK) >>> n);
                default: return null;
                }}));
    }
    private static final
    ...
    IntVector lanewise(VectorOperators.Binary op,
                                  int e) {
        if (opKind(op, VO_SHIFT) && e == e) {
            return lanewiseShift(op, e);
        }
        if (op == AND_NOT) {
            op = AND; e = ~e;
        }
   ...
   /**
    ...
    IntVector sliceTemplate(int origin, Vector<Integer> v1) {
        ...
        VectorMask<Integer> blendMask = iota.toVector().compare(VectorOperators.LT, (broadcast(length() - origin)));
    ...
    IntVector rearrangeTemplate(Class<S> shuffletype,
        ...
        return broadcast(0).blend(unmasked, m);
    }
    ...
    static final class IntSpecies extends AbstractSpecies<Integer> {
        ...
        @Override
        @ForceInline
        Class<Integer> genericElementType() {
        ...
        @Override
        @ForceInline
        public Class<? extends IntVector> vectorType() {
        ...
        @Override
        @ForceInline
        IntVector broadcastBits(long bits) {
            return (IntVector)
        ...
        @Override
        @ForceInline
        IntVector fromIntValues(int[] values) {
        ...
        @Override
        @ForceInline
        public VectorMask<Integer> maskAll(boolean bit) {
            if (vectorType() == IntMaxVector.class)
