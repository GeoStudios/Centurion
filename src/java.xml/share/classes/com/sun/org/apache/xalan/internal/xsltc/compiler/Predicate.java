...
import java.xml.share.classes.com.sun.org.apache.xalan.internal.xsltc.compiler.util.FilterGenerator;
...
final class Predicate extends Expression implements Closure {
    ...
    public int getPosType() {
        if (_ptype == -1) {
            ...
            else if (parent instanceof AbsoluteLocationPath path) {
                Expression exp = path.getPath();
                if (exp instanceof Step) {
                    _ptype = ((Step)exp).getNodeType();
                }
            }
            else if (parent instanceof VariableRefBase ref) {
   ...
   /**
    ...
    public Step getStep() {
        ...
        if (_exp instanceof EqualityExpr exp) {
