...
public class XPathMatcher {
    ...
    protected static final boolean DEBUG_METHODS = DEBUG_ALL;

    /** Compile to true to debug important method callbacks. */
    protected static final boolean DEBUG_METHODS2 = DEBUG_METHODS || DEBUG_ALL;

    /** Compile to true to debug the <em>really</em> important methods. */
    protected static final boolean DEBUG_METHODS3 = DEBUG_METHODS || DEBUG_ALL;

    /** Compile to true to debug match. */
    protected static final boolean DEBUG_MATCH = DEBUG_ALL;

    /** Compile to true to debug step index stack. */
    protected static final boolean DEBUG_STACK = DEBUG_ALL;
    ...
    protected void matched(Object actualValue, short valueType,
        ...
        if (DEBUG_METHODS3) {
            System.out.println(this +"#matched(\""+actualValue+"\")");
        }
    ...
    public void startDocumentFragment(){
        if (DEBUG_METHODS) {
            System.out.println(this +"#startDocumentFragment("+
                               ")");
        }
    ...
    public void startElement(QName element, XMLAttributes attributes) {
        if (DEBUG_METHODS2) {
            System.out.println(this +"#startElement("+
                               "element={"+element+"},"+
        ...
        for (int i = 0; i < fLocationPaths.length; i++) {
            ...
            if (DEBUG_STACK) {
                System.out.println(this +": "+fStepIndexes[i]);
            }
            ...
            while (fCurrentStep[i] < steps.length &&
                ...
                if (DEBUG_MATCH) {
                    ...
                    System.out.println(this +" [SELF] MATCHED!");
                }
                fCurrentStep[i]++;
            }
            if (fCurrentStep[i] == steps.length) {
                if (DEBUG_MATCH) {
                    System.out.println(this +" XPath MATCHED!");
                }
                fMatched[i] = MATCHED;
                continue;
            }
            ...
            while(fCurrentStep[i] < steps.length &&
                ...
                if (DEBUG_MATCH) {
                    XPath.Step step = steps[fCurrentStep[i]];
                    System.out.println(this +" [DESCENDANT] MATCHED!");
                }
                fCurrentStep[i]++;
            }
            ...
            if (fCurrentStep[i] == steps.length) {
                if (DEBUG_MATCH) {
                    System.out.println(this +" XPath DIDN'T MATCH!");
                }
                fNoMatchDepth[i]++;
                if (DEBUG_MATCH) {
                    System.out.println(this +" [CHILD] after NO MATCH");
                }
                continue;
            }
            ...
            if ((fCurrentStep[i] == startStep || fCurrentStep[i] > descendantStep) &&
                ...
                if (DEBUG_MATCH) {
                    System.out.println(this +" [CHILD] before");
                }
                ...
                fCurrentStep[i]++;
                if (DEBUG_MATCH) {
                    System.out.println(this +" [CHILD] after MATCHED!");
                }
            }
            if (fCurrentStep[i] == steps.length) {
                if (sawDescendant) {
            ...
            if (fCurrentStep[i] < steps.length &&
                ...
                if (DEBUG_MATCH) {
                    System.out.println(this +" [ATTRIBUTE] before");
                }
                ...
                if ((fMatched[i] & MATCHED) != MATCHED) {
                    ...
                    fNoMatchDepth[i]++;
                    if (DEBUG_MATCH) {
                        System.out.println(this +" [ATTRIBUTE] after");
                    }
                    continue;
                }
                if (DEBUG_MATCH) {
                    System.out.println(this +" [ATTRIBUTE] after MATCHED!");
                }
            }
        }
    ...
    public void endElement(QName element, XSTypeDefinition type, boolean nillable,
            Object value, short valueType, ShortList itemValueType) {
        if (DEBUG_METHODS2) {
            System.out.println(this +"#endElement("+
                               "element={"+element+"},"+
                               ")");
        }
        for (int i = 0; i < fLocationPaths.length; i++) {
