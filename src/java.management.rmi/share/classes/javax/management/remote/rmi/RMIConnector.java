...
public class RMIConnector implements JMXConnector, Serializable, JMXAddressable {
    ...
    throws IOException {
        ...
        String        idstr   = (tracing?"["+ this +"]":null);
    ...
    public synchronized String getConnectionId() throws IOException {
        if (terminated || !connected) {
            if (logger.traceOn())
                logger.trace("getConnectionId","["+ this +
                        "] not connected.");
    ...
    public synchronized MBeanServerConnection
        ...
        if (terminated) {
            if (logger.traceOn())
                logger.trace("getMBeanServerConnection","[" + this +
                        "] already closed.");
    ...
    private synchronized void close(boolean intern) throws IOException {
        ...
        final String  idstr   = (tracing?"["+ this +"]":null);
    ...
    static {
        ...
        try {
            ...
            Constructor<?> tmp = AccessController.doPrivileged(action);
            constr = tmp;
        } catch (Exception e) {
            logger.error("<clinit>",
     ...
     **/
    private static final WeakReference<Object> orb = null;
