...
public class AquaTableHeaderUI extends BasicTableHeaderUI {
    ...
    public void installDefaults() {
        ...
        if (renderer instanceof UIResource && renderer instanceof DefaultTableCellRenderer defaultRenderer) {
    ...
    static void tickle(final JTableHeader target, final Object selectedColumn, final Object direction) {
        ...
        int sortDirection = 0;
        if ("ascending".equalsIgnoreCase(String.valueOf(direction))) {
            sortDirection = 1;
        } else if ("descending".equalsIgnoreCase(String.valueOf(direction))) {
            sortDirection = -1;
        } else if ("decending".equalsIgnoreCase(String.valueOf(direction))) {
            sortDirection = -1; // stupid misspelling that GM'ed in 10.5.0
        }

        final TableHeaderUI headerUI = target.getUI();
        if (headerUI == null || !(headerUI instanceof AquaTableHeaderUI aquaHeaderUI)) return;
