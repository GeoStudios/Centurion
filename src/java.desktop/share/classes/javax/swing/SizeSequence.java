...
public class SizeSequence {
    ...
    public void insertEntries(int start, int length, int value) {
        ...
        a = new int[n];
        if (start >= 0) System.arraycopy(sizes, 0, a, 0, start);
        for (int i = start; i < end; i++) {
            a[i] = value ;
        }
        if (n - end >= 0) System.arraycopy(sizes, end - length, a, end, n - end);
        setSizes(a);
    }
    ...
    public void removeEntries(int start, int length) {
        ...
        a = new int[n];
        if (start >= 0) System.arraycopy(sizes, 0, a, 0, start);
        if (n - start >= 0) System.arraycopy(sizes, start + length, a, start, n - start);
        setSizes(a);
    }
}
