...
public interface PixelDrawPipe {
    void drawLine(SunGraphics2D sg,
                         int x1, int y1, int x2, int y2);

    void drawRect(SunGraphics2D sg,
                         int x, int y, int width, int height);

    void drawRoundRect(SunGraphics2D sg,
                              int x, int y, int width, int height,
                              int arcWidth, int arcHeight);

    void drawOval(SunGraphics2D sg,
                         int x, int y, int width, int height);

    void drawArc(SunGraphics2D sg,
                        int x, int y, int width, int height,
                        int startAngle, int arcAngle);

    void drawPolyline(SunGraphics2D sg,
                             int[] xPoints, int[] yPoints,
                             int nPoints);

    void drawPolygon(SunGraphics2D sg,
                            int[] xPoints, int[] yPoints,
                            int nPoints);
}
