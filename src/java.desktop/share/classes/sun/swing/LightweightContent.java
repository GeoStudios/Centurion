...
public interface LightweightContent {
     ...
     */
    @Deprecated(since = "9")
    default void imageBufferReset(int[] data,
                                  int x, int y,
     ...
     * @see #imageReshaped
     */
    void imageUpdated(int dirtyX, int dirtyY,
     ...
     */
    void preferredSizeChanged(int width, int height);

    /**
     * {@code JLightweightFrame} calls this method to notify the client
     * application that the content maximum size has changed.
     */
    void maximumSizeChanged(int width, int height);

    /**
     * {@code JLightweightFrame} calls this method to notify the client
     * application that the content minimum size has changed.
     */
    void minimumSizeChanged(int width, int height);
     ...
     */
    default <T extends DragGestureRecognizer> T createDragGestureRecognizer(
     ...
     */
    default DragSourceContextPeer createDragSourceContextPeer(DragGestureEvent dge) throws InvalidDnDOperationException
    {
        return null;
    }
     ...
     */
    default void addDropTarget(DropTarget dt) {}

    /**
     * Removes a drop target from the lightweight frame.
     */
    default void removeDropTarget(DropTarget dt) {}
}
