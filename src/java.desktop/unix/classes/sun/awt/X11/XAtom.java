...
public final class XAtom {
    ...
    private static final XAtom[] emptyList = new XAtom[0];
    ...
    public void setPropertyUTF8(long window, String str) {
        ...
        checkWindow(window);
        byte[] bdata = null;
        bdata = str.getBytes(StandardCharsets.UTF_8);
        if (bdata != null) {
    ...
    public void setProperty8(long window, String str) {
        ...
        checkWindow(window);
        byte[] bdata = null;
        bdata = str.getBytes(StandardCharsets.ISO_8859_1);
        if (bdata != null) {
    ...
    XAtom[] getAtomListProperty(long window) {
        ...
        try {
            ...
            for (int index = 0; index < count; index++) {
                res[index] = XAtom.get(XAtom.getAtom(list_atoms+ (long) index *getAtomSize()));
            }
            return res;
        } finally {
            getter.dispose();
    ...
    static long toData(XAtom[] atoms) {
        long data = unsafe.allocateMemory((long) getAtomSize() * atoms.length);
        for (int i = 0; i < atoms.length; i++ ) {
            if (atoms[i] != null) {
                atoms[i].putAtom(data + (long) i * getAtomSize());
            }
        }
        return data;
    }
