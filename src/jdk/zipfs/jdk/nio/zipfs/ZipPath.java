...
final class ZipPath implements Path {
    ...
    ZipPath(jdk.nio.zipfs.ZipFileSystem zfs, byte[] path) {
        this(zfs, path, false);
    }

    ZipPath(jdk.nio.zipfs.ZipFileSystem zfs, byte[] path, boolean normalized) {
        this.zfs = zfs;
        if (normalized) {
            this.path = path;
        } else {
    ...
    ZipPath(jdk.nio.zipfs.ZipFileSystem zfs, String path) {
        this.zfs = zfs;
        this.path = normalize(path);
    }
    ...
    public jdk.nio.zipfs.ZipPath getFileName() {
        ...
        return new jdk.nio.zipfs.ZipPath(getFileSystem(), result, true);
    }
    ...
    @Override
    public jdk.nio.zipfs.ZipPath getName(int index) {
        initOffsets();
        ...
        return new jdk.nio.zipfs.ZipPath(zfs, result);
    }

    @Override
    public jdk.nio.zipfs.ZipPath subpath(int beginIndex, int endIndex) {
        initOffsets();
        if (beginIndex < 0 ||
    ...
    @Override
    public jdk.nio.zipfs.ZipPath toRealPath(LinkOption... options) throws IOException {
        jdk.nio.zipfs.ZipPath realPath;
        byte[] resolved = getResolvedPath();
        // resolved is always absolute and normalized
        if (resolved == path) {
            realPath = this;
        } else {
            realPath = new jdk.nio.zipfs.ZipPath(zfs, resolved, true);
    ...
    public jdk.nio.zipfs.ZipPath toAbsolutePath() {
        if (isAbsolute()) {
            ...
            tmp[0] = '/';
            return new jdk.nio.zipfs.ZipPath(zfs, tmp, true);  // normalized
        }
    }
    ...
    private boolean equalsNameAt(jdk.nio.zipfs.ZipPath other, int index) {
    ...
    public Path relativize(Path other) {
        final jdk.nio.zipfs.ZipPath o = checkPath(other);
        if (o.equals(this))
            return new jdk.nio.zipfs.ZipPath(zfs, new byte[0], true);
        if (this.path.length == 0)
            return o;
        if (this.zfs != o.zfs || this.isAbsolute() != o.isAbsolute())
            throw new IllegalArgumentException();
        if (this.path.length == 1 && this.path[0] == '/')
            return new jdk.nio.zipfs.ZipPath(zfs,
    ...
    @Override
    public jdk.nio.zipfs.ZipPath resolve(Path other) {
        jdk.nio.zipfs.ZipPath o = checkPath(other);
        if (o.path.length == 0)
            return this;
    ...
    private jdk.nio.zipfs.ZipPath resolve(byte[] opath) {
        byte[] resolved;
        ...
        return new jdk.nio.zipfs.ZipPath(zfs, resolved, true);
    }
    ...
    public boolean startsWith(Path other) {
        Objects.requireNonNull(other, "other");
        if (!(other instanceof final jdk.nio.zipfs.ZipPath o))
            return false;
    ...
    @Override
    public jdk.nio.zipfs.ZipPath resolve(String other) {
        byte[] opath = normalize(other);
        if (opath.length == 0)
            return this;
        if (opath[0] == '/' || this.path.length == 0)
            return new jdk.nio.zipfs.ZipPath(zfs, opath, true);
    ...
    private jdk.nio.zipfs.ZipPath checkPath(Path path) {
        Objects.requireNonNull(path, "path");
        if (!(path instanceof jdk.nio.zipfs.ZipPath))
            throw new ProviderMismatchException();
        return (jdk.nio.zipfs.ZipPath) path;
    }
    ...
    public boolean equals(Object obj) {
        return obj instanceof jdk.nio.zipfs.ZipPath &&
                this.zfs == ((jdk.nio.zipfs.ZipPath) obj).zfs &&
    ...
    <V extends FileAttributeView> V getFileAttributeView(Class<V> type) {
        ...
        if (type == BasicFileAttributeView.class)
            return (V)new jdk.nio.zipfs.ZipFileAttributeView(this, false);
        if (type == jdk.nio.zipfs.ZipFileAttributeView.class)
            return (V)new jdk.nio.zipfs.ZipFileAttributeView(this, true);
        if (zfs.supportPosix) {
    ...
    private jdk.nio.zipfs.ZipFileAttributeView getFileAttributeView(String type) {
        if (type == null)
            throw new NullPointerException();
        if ("basic".equals(type))
            return new jdk.nio.zipfs.ZipFileAttributeView(this, false);
        if ("zip".equals(type))
            return new jdk.nio.zipfs.ZipFileAttributeView(this, true);
        if (zfs.supportPosix) {
    ...
    DirectoryStream<Path> newDirectoryStream(DirectoryStream.Filter<? super Path> filter)
            throws IOException
    {
        return new jdk.nio.zipfs.ZipDirectoryStream(this, filter);
    }
    ...
    jdk.nio.zipfs.ZipFileAttributes readAttributes() throws IOException {
        jdk.nio.zipfs.ZipFileAttributes zfas = zfs.getFileAttributes(getResolvedPath());
        if (zfas == null)
    ...
    <A extends BasicFileAttributes> A readAttributes(Class<A> type) throws IOException {
        ...
        if (type == BasicFileAttributes.class || type == jdk.nio.zipfs.ZipFileAttributes.class) {
    ...
    jdk.nio.zipfs.ZipFileAttributes readAttributesIfExists() throws IOException {
    ...
    boolean isSameFile(Path other) throws IOException {
        ...
        if (other == null ||
            ...
            return false;
        this.checkAccess();
        ((jdk.nio.zipfs.ZipPath)other).checkAccess();
        return Arrays.equals(this.getResolvedPath(),
                ((jdk.nio.zipfs.ZipPath)other).getResolvedPath());
    }
    ...
    void move(jdk.nio.zipfs.ZipPath target, CopyOption... options)
            throws IOException
    {
    ...
    void copy(jdk.nio.zipfs.ZipPath target, CopyOption... options)
            throws IOException
    {
        if (Files.isSameFile(this.zfs.getZipFile(), target.zfs.getZipFile()))
            zfs.copyFile(false,
    ...
    private void copyToTarget(jdk.nio.zipfs.ZipPath target, CopyOption... options)
            throws IOException
    {
        ...
        jdk.nio.zipfs.ZipFileAttributes zfas = readAttributes();
        ...
        if (copyAttrs) {
            jdk.nio.zipfs.ZipFileAttributeView view =
                    target.getFileAttributeView(jdk.nio.zipfs.ZipFileAttributeView.class);
            try {
