...
public class Plotter extends JComponent
    ...
    private final TimeStamps times = new TimeStamps();
    private final ArrayList<Sequence> seqs = new ArrayList<Sequence>();
    ...
    private final Rectangle r = new Rectangle(1, 1, 1, 1);
    private Font smallFont = null;

    // Initial margins, may be recalculated as needed
    private final int topMargin = 10;
    private final int bottomMargin = 45;
        ...
        try {
            ...
            if (seqs.size() > 0 && seqs.get(0).size > 0) {
                for (int i = 0; i < seqs.get(0).size; i++) {
                    ...
                    out.printf(Locale.ENGLISH, "%.6f", excelTime);
                    for (Sequence seq : seqs) {
    ...
    public void paintComponent(Graphics g) {
        ...
        if (viewRange > 0) {
            ...
            viewRangeMS = Math.max(tMax - tMin, MINUTE);
        }
        ...
        int n = 5;
        if ((String.valueOf(vMax)).startsWith("2")) {
            n = 4;
        } else if ((String.valueOf(vMax)).startsWith("3")) {
            n = 6;
        } else if ((String.valueOf(vMax)).startsWith("4")) {
            n = 4;
        } else if ((String.valueOf(vMax)).startsWith("6")) {
            n = 6;
        } else if ((String.valueOf(vMax)).startsWith("7")) {
            n = 7;
        } else if ((String.valueOf(vMax)).startsWith("8")) {
            n = 8;
        } else if ((String.valueOf(vMax)).startsWith("9")) {
            n = 3;
        }
    ...
    public void propertyChange(PropertyChangeEvent ev) {
        ...
        if (prop == JConsoleContext.CONNECTION_STATE_PROPERTY) {
            ...
            if (Objects.requireNonNull(newState) == ConnectionState.DISCONNECTED) {
                synchronized (this) {
