...
public final class LdapClient implements PooledConnection {
    ...
    private static byte[] encodePassword(Object pw, boolean v3) throws IOException {
        ...
        if (pw instanceof String) {
            if (v3) {
                return ((String)pw).getBytes(StandardCharsets.UTF_8);
            } else {
                return ((String)pw).getBytes(StandardCharsets.ISO_8859_1);
            }
        } else {
            return (byte[])pw;
        }
            ...
            ber.beginSeq(Ber.ASN_SEQUENCE | Ber.ASN_CONSTRUCTOR);
                ...
                ber.beginSeq(LDAP_REQ_COMPARE);
                    ...
                    ber.beginSeq(Ber.ASN_SEQUENCE | Ber.ASN_CONSTRUCTOR);
                        ...
                        byte[] val = isLdapv3 ?
                            value.getBytes(StandardCharsets.UTF_8) : value.getBytes(StandardCharsets.ISO_8859_1);
